...
For this part, where it will be considered the value of $v_s(t)$ when $t > 0$, another node analysis was performed. The equations used were the same as the ones already presented in Section~\ref{sec:node}, but in this case, $I_c$ is not zero. For this situation $I_c = \frac{V_6-V_8}{Z}$, where $Z = \frac{1}{jwC}$ is the impedance of the capacitor, that has capacitance C. $w = 2\pi f$ is the radial frequency of the signal of $v_s(t) = \sin(wt) = \cos(wt-\frac{\pi}{2})$. This sinusoidal excitation will induce a sinusoidal forced solution with the same frequency for the components of the circuit.

Consequently, the voltages in the nodes for the forced solution will have the form of $V_i\cos(wt-\phi_i)$. It is possible to compute the voltages in the nodes by representing these voltages as phasors. Solving the matrix below, with {\bf Octave}, gives the values of the phasors, that are in table \ref{forced_res}. The module of these complex numbers gives the amplitude $V_i$ and their angle is equal to $\frac{pi}{2}-\phi_i$.
